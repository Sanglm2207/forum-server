...
const resolvers: IResolvers = {
  ...
  Query: {
    ...
    me: async (
      ...
      let user: UserResult;
      try {
        //TODO: May should extract userId
        if (!(ctx.req.session as any)?.userId) {
          return {
            messages: ["User not logged in."],
          };
        }
        user = await me((ctx.req.session as any).userId);
        if (user && user.user) {
          return user.user;
        }
        return {
  ...
  Mutation: {
    ...
    login: async (
      ...
      try {
        ...
        if (user && user.user) {
          (ctx.req.session as any)!.userId = user.user.id;
    ...
    logout: async (
      ...
      try {
        ...
        ctx.req.session?.destroy((err: any) => {
          ...
          console.log("session destroyed", (ctx.req.session as any)?.userId);
        });
        return result;
      } catch (ex) {
        throw ex;
